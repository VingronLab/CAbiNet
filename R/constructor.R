...
#' Check if cacontainer object was correctly created.
#'
#' @description Checks if the slots in a cacontainer object are of the correct size
#' and whether they are coherent.
#' @param object A cacontainer object.
#' @return TRUE if it is a valid cacontainer object. FALSE otherwise.
#' @export
#' @examples
...
#'
#' check_cacontainer(ca)
check_cacontainer <- function(object) {
  errors <- character()
...
#' @name cacontainer-class
#' @rdname cacontainer-class
#' @description
...
setClass("cacontainer",
         ...
         prototype(
           ...
           col_inertia = numeric(),
           permuted_data = list()),
         validity = check_cacontainer
)

#' Create new "cacontainer" object.
#' @description Creates new cacontainer object.
#'
#' @param ... slot names and objects for new cacontainer object.
#' @return cacontainer object
#' @rdname cacontainer-class
#' @export
#' @examples
#' set.seed(1234)
...
#' SVD <- SVD[c(2, 1, 3)]
#'
#' ca <- new_cacontainer(U = SVD$U,
#'                  V = SVD$V,
#'                  D = SVD$D,
#'                  row_masses = res$rowm,
#'                  col_masses = res$colm)
new_cacontainer <- function(...) new("cacontainer",...)


#' Access slots in a cacontainer object
#' 
#' @param caobj a cacontainer object
#' @param slot slot to return
#' @returns Chosen slot of the cacontainer object
#' @examples 
#' # Simulate scRNAseq data.
#' cnts <- data.frame(cell_1 = rpois(10, 5),
#'                    cell_2 = rpois(10, 10),
...
#' cacontainer_slot(ca, "U")
#' 
#' @export
cacontainer_slot <- function(caobj, slot){
  stopifnot(slot %in% slotNames(caobj))
  
  return(slot(caobj, slot))
}

#' Prints slot names of cacontainer object
#' 
#' @param caobj a cacontainer object
#' @returns Prints slot names of cacontainer object
#' @examples 
#' # Simulate scRNAseq data.
#' cnts <- data.frame(cell_1 = rpois(10, 5),
#'                    cell_2 = rpois(10, 10),
...
#' 
#' # show slot names:
#' cacontainer_names(ca)
#' 
#' @export
cacontainer_names <- function(caobj){
  slotNames(caobj)
}