...
```
After building the  cell-gene graph, the biclustering is performed with either the Leiden algorithm (`algorithm = "leiden"`) or Spectral clustrering (`algorithm = "spectral"`). For the up-to-date version of our package, we call "leiden" function from either R package BiocStyle::Biocpkg('leiden') or BiocStyle::Biocpkg('igraph'). The BiocStyle::Biocpkg('igraph') implementation can be applied by set "leiden_pack = 'igraph'" (as default, see ?caclust). From our testing, the BiocStyle::Biocpkg('igraph') implementation runs faster than package BiocStyle::Biocpkg('leiden') with the similar accuracy. Therefore, we recommend to run  
