...
install_miniconda()
use_condaenv(
  condaenv = file.path(miniconda_path(), "envs/r-reticulate"),
  required = TRUE
)
...
The number of genes kept should not be set too low, as otherwise important marker genes are often removed from the data.
...
```
After building the  cell-gene graph, the biclustering is performed with either the Leiden algorithm (`algorithm = "leiden"`) or Spectral clustrering (`algorithm = "spectral"`). For the up-to-date version of the package, we call the "leiden" function from either the R package `r BiocStyle::Rpackage('leiden')` or `r BiocStyle::Rpackage('igraph')`. The `r BiocStyle::Rpackage('igraph')` implementation can be applied by set "leiden_pack = 'igraph'" (as default, see ?caclust). From our testing, the `r BiocStyle::Rpackage('igraph')` implementation runs faster than package `r BiocStyle::Rpackage('leiden')` with similar accuracy. Therefore, we recommend to set `leiden_pack = "igraph"`.
...
If you do not have a list of known marker genes for the cell types in your data, you can use the `annotate_biclustering()` function. It uses the "CellMarker" gene set to automatically annotate the biclusters with the best matching cell type in the set:
```{r auto_anno}
ann_cabic <- annotate_biclustering(
  obj = cabic,
...
Last but not least, you can explore the visualization interactively, or just display the names of the marker genes in the plot. Co-clustered genes can be labeled in the biMAP. For example, if you don't have a list of known marker genes at hand, you can display the names of potential marker genes by
```{r label_mkg}
plot_biMAP(
  cabic,
  color_by = "cluster",
...
scater::plotExpression(sce,
  features = markers[markers %in% rownames(sce)],
