...
CAbiNet allows for convenient cell type annotation. Instead of having to make a large number of feature plots, we can simply display the position of a set of marker genes directly in the biMAP.
...
Sometimes the name of genes overlap with each other and can be difficult to read the names in the plot. Exploring the biMAP interactively allows you to zoom in or out at a group of cells to inspect the genes there in more detail:
```{r label_mkg_live}
plot_biMAP(cabic, color_by = "cluster",
           color_genes = TRUE,
           interactive = TRUE)
