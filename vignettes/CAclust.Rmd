...
CAbiNet is designed to work with either a count matrix or to be compatible with `r BiocStyle::Rpackage("SingleCellExperiment")` object. We will take one dataset from `r BiocStyle::Rpackage('scRNAseq')` as an example to illustrate how to work with our algorithm and package.
```{r load_data}
...
plot_biMAP(bicabic, color_by = "cluster",
           ...
           color_genes = TRUE)
```
The biMAP can also be explored in an interactive way. The identities of genes and cells will be displayed when  you mouse over the points.
```{r label_mkg}
plot_biMAP(bicabic, color_by = "cluster",
           color_genes = TRUE,
           interactive = TRUE)
...
```{r biMAP_feature}
plot_feature_biMAP(sce = sce,caclust = bicabic, feature = 'EGR1')
```
...
CAbiNet accepts both a gene-by-cell count matrix and a `r BiocStyle::Rpackage("SingleCellExperiment")` object with the counts. If you starts with
